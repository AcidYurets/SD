...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withEvents; query != nil {
		if err := tq.loadEvents(ctx, query, nodes,
			func(n *Tag) { n.Edges.Events = []*Event{} },
			func(n *Tag, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Event)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.EventsTable)
		s.Join(joinT).On(s.C(event.FieldID), joinT.C(tag.EventsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.EventsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.EventsPrimaryKey[1]))
